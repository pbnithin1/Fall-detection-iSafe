...
while True:
    ...
    if results.pose_landmarks:
        ...
        cv2.polylines(color_image, [np.array(feet_points, dtype=np.int32)], isClosed=True, color=(0, 255, 0), thickness=2)
        ...
        # Check if the CoG is outside the quadrilateral
        if (
            cog_x < min(feet_points[0][0], feet_points[1][0], feet_points[2][0], feet_points[3][0]) or
            cog_x > max(feet_points[0][0], feet_points[1][0], feet_points[2][0], feet_points[3][0])
        ):
            cv2.putText(depth_image, "FALL IMMINENT", (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3)
